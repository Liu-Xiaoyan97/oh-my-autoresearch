...
- **observer**：独立持久化 sidecar，负责所有日志/数据库/状态机/knowledge 的落盘。
  主程序和 subagent 只能通过 `emit_event.py` 提交事件（fire-and-forget），observer 自行消费写入。
  禁止直接写文件、调 observer 内部脚本或读其内部产物。
  - observer 有独立 LLM 配置，`current_step=9` 时 autonomously 生成 observation 并追加 knowledges。
