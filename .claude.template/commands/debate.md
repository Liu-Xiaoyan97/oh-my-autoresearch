...
Before running the command, release every non-lead project agent. Read
...
then send `shutdown_request` only to the remaining project agents. Do not send
shutdown to `team-lead` and do not wait for its approval. Require each target to
reply via `SendMessage` with a structured object-form shutdown approval, then
call `TeamDelete`. Do not send protocol JSON as a string: use `summary:
...
shutdown protocol frame by the target agent. In
in-process mode, the CLI panel is rendered from the team/task
