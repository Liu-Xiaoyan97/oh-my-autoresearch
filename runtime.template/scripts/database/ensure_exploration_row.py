#!/usr/bin/env python3
"""ensure_exploration_row.py - 保证 exploration 表中存在某个 exp_name 行。"""
...
def main():
    ...
    if not exp_name:
        print("用法: ensure_exploration_row.py <exp_name>", file=sys.stderr)
        sys.exit(1)
    ...
    conn.commit()
    conn.close()
    print(f"Ensured exploration row: {exp_name}")
