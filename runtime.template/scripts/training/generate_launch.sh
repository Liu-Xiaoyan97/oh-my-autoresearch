...
PY
    ...
    TRAINING_ARGS="$(python3 -c 'import json,shlex,sys; print(" ".join(shlex.quote(str(x)) for x in json.loads(sys.argv[1])["args"]))' "$METADATA")"
...
case "\$SCRIPT_PATH" in
    *.py)
        exec python "\$SCRIPT_PATH" "\${TRAINING_ARGS[@]}"
        ;;
    *.sh)
        exec bash "\$SCRIPT_PATH" "\${TRAINING_ARGS[@]}"
        ;;
    *)
        if [[ -x "\$SCRIPT_PATH" ]]; then
            exec "\$SCRIPT_PATH" "\${TRAINING_ARGS[@]}"
        else
            exec bash "\$SCRIPT_PATH" "\${TRAINING_ARGS[@]}"
        fi
        ;;
