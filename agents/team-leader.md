...
10. **Signal the orchestrator to release the team and advance.** After the file
    ...
    TEARDOWN_REQUIRED: Exclude the main/orchestrator team-lead member (agentId == leadAgentId, agentType == "team-lead", or name == "team-lead"); send shutdown_request only to non-lead project agents using SendMessage structured object form with summary + message.type; require each target to SendMessage structured object form {"type":"shutdown_response","approve":true}. Do NOT busy-wait with foreground sleep — end the turn and let shutdown_response wake the main turn, or poll via CronCreate (never sleep). Bound the graceful wait to 2 minutes; for any non-lead target that has not emitted a real structured approval by then (flash agents often narrate "approved" without actually emitting the frame), force-terminate it with TaskStop on its task_id recorded at spawn (or discovered via TaskList) — TaskStop does not depend on the agent's cooperation and is what makes TeamDelete (which fails while members are active) succeed. Then TeamDelete; then verify the team/task dirs disappeared (cron-poll, no sleep) and only rm -rf ~/.claude/teams/<team_name> and ~/.claude/tasks/<team_name> if they still exist after that.
