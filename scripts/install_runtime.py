...
def install_runtime(
    ...
    for rel_dir in manifest.ensure_dirs:
        ...
        if path.is_dir():
            continue
        actions.append(f"create dir {rel_dir}")
        if not dry_run:
    ...
    if not dry_run:
        ...
        if errors:
            raise RuntimeError("Installed runtime failed validation:\n" + "\n".join(f"- {e}" for e in errors))

    return actions


def main() -> int:
    parser = argparse.ArgumentParser(description="Initialize a research-runtime repository from templates.")
    ...
    for action in actions:
        print(action)
    print("Runtime install dry-run complete." if args.dry_run else "Runtime install complete.")
    return 0
